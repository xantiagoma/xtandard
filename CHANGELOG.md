# Changelog

## v0.2.0

[compare changes](https://github.com/xantiagoma/xantiagoma-lib/compare/v0.1.2...v0.2.0)

### 🚀 Enhancements

- Add pagination utilities ([d6eddcf](https://github.com/xantiagoma/xantiagoma-lib/commit/d6eddcf))

### 🩹 Fixes

- Skip GitHub Release if already exists ([801bd43](https://github.com/xantiagoma/xantiagoma-lib/commit/801bd43))

### ❤️ Contributors

- Santiago Montoya ([@xantiagoma](https://github.com/xantiagoma))

## v0.1.2

[compare changes](https://github.com/xantiagoma/xantiagoma-lib/compare/v0.1.1...v0.1.2)

### 🚀 Enhancements

- Add usePreventAutoFocus hook to xantiagoma/react ([1b3fc18](https://github.com/xantiagoma/xantiagoma-lib/commit/1b3fc18))
- Add createHttpInterceptor to xantiagoma/web ([9b71af7](https://github.com/xantiagoma/xantiagoma-lib/commit/9b71af7))
- Add useDynamicRefs hook + createHttpInterceptor + README with source links ([d7d9901](https://github.com/xantiagoma/xantiagoma-lib/commit/d7d9901))

### 🩹 Fixes

- Suppress browser open on release (--no-open) ([b3563bd](https://github.com/xantiagoma/xantiagoma-lib/commit/b3563bd))

### ❤️ Contributors

- Santiago Montoya ([@xantiagoma](https://github.com/xantiagoma))

## v0.1.1

[compare changes](https://github.com/xantiagoma/xantiagoma-lib/compare/v0.1.0...v0.1.1)

### 🚀 Enhancements

- Add Providers + provider() to xantiagoma/react ([02df996](https://github.com/xantiagoma/xantiagoma-lib/commit/02df996))

### 📖 Documentation

- Expand recommended libraries from starred repos ([fc50883](https://github.com/xantiagoma/xantiagoma-lib/commit/fc50883))
- Add more recommended libraries ([2290343](https://github.com/xantiagoma/xantiagoma-lib/commit/2290343))
- Add dexie ([3398198](https://github.com/xantiagoma/xantiagoma-lib/commit/3398198))

### ❤️ Contributors

- Santiago Montoya ([@xantiagoma](https://github.com/xantiagoma))
